...
把用户私聊转发到 Telegram 群组 Topic，支持协管权限、D1 数据库、验证系统、CRM 资料卡、黑名单隔离、聚合收件箱、网页后台和 AI 翻译。
...
KV 用于保存临时验证、冷却、兼容旧数据、Topic 映射缓存等状态。
...
| 类型 | 推荐变量名 | 兼容变量名 |
| --- | --- | --- |
| D1 database | `DB` | `D1`、`DATABASE`、`RELAYGO_DB` |
...
**RelayGo — 用 Cloudflare Workers 搭建轻量、可控、免服务器的 Telegram 客服中继系统。**
